...
const handleRegister = async (requestBody) => {
  ...
  try {
    ...
    const [existingEmails] = await connection.execute(queries.getUserByEmail, [
      email,
    ]);
...
const handleValidateResetToken = async (requestBody) => {
  ...
  try {
    ...
    const [resets] = await connection.execute(queries.getValidPasswordReset, [
      resetToken,
      new Date(),
    ]);
...
const handlePasswordReset = async (requestBody) => {
  ...
  try {
    ...
    const [users] = await connection.execute(queries.getUserByEmail, [email]);
    ...
    // Store reset token
    await connection.execute(queries.createPasswordReset, [
      user.user_id,
      resetToken,
    ]);
...
const handlePasswordResetConfirm = async (requestBody) => {
  ...
  try {
    ...
    await connection.execute(queries.updateUserPassword, [
      passwordHash,
      reset.user_id,
    ]);

    // Mark reset token as used
    await connection.execute(queries.markResetTokenUsed, [resetToken]);
